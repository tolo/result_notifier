  ...
  group('NotifierStore', () {
    test('Creates ResultNotifier', () async {
      final store = ResultStore<String, String>(
          create: (key, _) => ResultNotifier<String>(data: 'data$key'));
    ...
    test('Auto-disposes stale ResultNotifier', () async {
      ...
      void listener() => invokeListenerCount++;
      final store = ResultStore<String, String>(
        create: (key, store) => ResultNotifier<String>(
            data: 'data$key', expiration: store.autoDisposeTimerInterval),
    ...
    test('OnResult is correctly invoked', () async {
      ...
      final store = ResultStore<String, String>(
          create: (key, store) => ResultNotifier<String>(data: 'data$key'));
      ...
      await Future.delayed(
          const Duration(milliseconds: 10)); // Wait for listener to be invoked
